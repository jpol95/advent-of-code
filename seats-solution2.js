...
const alterSeat = (seatArray, copy, x, y) => {
    ...
    for (let i = 0; i < seatsToCheck.length; i++){
        for (let j = 0; j < seatsToCheck.length; j++){
            if (i === 1 && j === 1) continue;
            let diagX = x + seatsToCheck[i]
            let diagY = y + seatsToCheck[j]
            while (!outOfBounds(copy, diagY, diagX)){
                if (seatArray[diagY][diagX] === '#' || seatArray[diagY][diagX] === 'L'){
                surroundArray.push(seatArray[diagY][diagX]);
                break;
                }
                diagX += seatsToCheck[i]
                diagY += seatsToCheck[j]
            }
        }
    }
    let occupiedSeats = 0;
    ...
    if (seatArray[y][x] === '#' && occupiedSeats > 4) copy[y][x] = 'L'
}
